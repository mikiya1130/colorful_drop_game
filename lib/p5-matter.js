...
var matter = (function () {
  ...
  var makeHexagon = function (x, y, diameter, options) {
    var hexagon = new Hexagon(x, y, diameter, options);
    addToWorld(hexagon);
    return hexagon;
  };
  ...
  function polygon(x, y, radius, npoints) {
    ...
    let angleHalf = angle / 2;
    beginShape();
    for (let a = angleHalf; a < TWO_PI + angleHalf; a += angle) {
  ...
  var Hexagon = function (x, y, diameter, options) {
    var body = Matter.Bodies.polygon(x, y, 6, diameter / 2, options);
    PhysicalObject.call(this, body, diameter, diameter);
    var sides = sides
  };
  Hexagon.prototype = Object.create(PhysicalObject.prototype);
  Hexagon.prototype.constructor = Hexagon;

  Hexagon.prototype.show = function () {
    push();
    translate(this.getPositionX(), this.getPositionY());
    rotate(this.getAngle());
    polygon(0, 0, this.getWidth() / 2, 6);
    circle(0, 0, this.getWidth() * 2 / 3);
    pop();
  };

  Hexagon.prototype.getDiameter = Hexagon.prototype.getWidth;
